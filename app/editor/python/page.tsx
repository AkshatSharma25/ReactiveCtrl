...
const EditorPage = () => {
    ...
    const handleSubmitCode = async () => {
        ...
        try {
            ...
            const response = await axios.post(`/api/submit/python`, { code: code,uid:user.uid })
            setFinalOutput(response.data.output)
        }
        catch (error) {
            toast.error("An error occurred while executing the code!")
            // setShowOutput(true);
            setOutputWindowVisiblity(false);
        }
        setTimeout(() => {
            setShowOutput(true);
        }, 2000);
            ...
            <div className={`flex flex-col  h-[100vh]`}>
                <div className='flex items-center justify-center'>
                    <div className={`absolute top-0 left-[25vw] ${outputWindowVisibility ? "" : "hidden"}`}>
                        ...
                        <OutputWindow visible={outputWindowVisibility} output={finalOutput} show={showOutput} language={"PYTHON"} />
                    </div>

                    <Navbar language={"Python"} tab={"Editor"} />
